...
export async function POST(req: NextRequest) {
  try {
    ...
    const { storeId, employeeId, templateId, lastWorkingDate, reason, tasks = [] } = body
    if (!storeId) return err('storeId required')
    ...
    if (templateId && tasks.length === 0) {
      const tmplRows = await query(`SELECT tasks FROM OnboardingTemplate WHERE id = ? AND type = 'OFFBOARDING'`, [templateId])
    ...
    const now = nowISO()
    await exec(
      `INSERT INTO EmployeeOffboarding (id, employeeId, storeId, templateId, status, lastWorkingDate, reason, tasks, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, 'IN_PROGRESS', ?, ?, ?, ?, ?)`,
      [id, employeeId, storeId, templateId ?? null, lastWorkingDate ?? null, reason ?? null, JSON.stringify(resolvedTasks), now, now],
    )
