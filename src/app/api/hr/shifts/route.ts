...
async function ensureTable() {
  await exec(`CREATE TABLE IF NOT EXISTS Shift (
    id TEXT PRIMARY KEY,
...
export async function GET(req: NextRequest) {
  try {
    ...
    let sql = `SELECT s.*, e.name as employeeName FROM Shift s
...
export async function POST(req: NextRequest) {
  try {
    await ensureTable()
    const body = await req.json() as { storeId?: string; employeeId?: string; date?: string; startTime?: string; endTime?: string; role?: string; notes?: string; status?: string }
    ...
    const now = nowISO()
    await exec(
      `INSERT INTO Shift (id, storeId, employeeId, date, startTime, endTime, role, notes, status, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
