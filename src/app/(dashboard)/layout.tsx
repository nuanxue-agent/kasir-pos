...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Resolve the active store: honour session cookie's activeStoreId if valid
  const activeStoreId: string | undefined = (user as any).activeStoreId
  const activeStore =
    (activeStoreId ? fullStores.find(s => s.id === activeStoreId) : undefined) ?? fullStores[0]
