...
function escPos(data: ReceiptData): Uint8Array {
  ...
  const text = (s: string) => { for (const c of s) buf.push(c.charCodeAt(0) & 0xFF) }
  ...
  if (data.storeAddress) { text(data.storeAddress.substring(0, 48)); nl() }
  if (data.storePhone) { text(data.storePhone); nl() }
  if (data.taxId) { text(`Tax ID: ${data.taxId}`); nl() }
  ...
  // Order info
  text(`No: ${data.orderNumber}`); nl()
  text(`${data.date}`); nl()
  if (data.cashier) { text(`Kasir: ${data.cashier}`); nl() }
  if (data.customerName) { text(`Pelanggan: ${data.customerName}`); nl() }
  ...
  for (const item of data.items) {
    ...
    text(qtyPrice + ' '.repeat(spaces) + subtotal); nl()
  }
  ...
  if (data.discountAmt && data.discountAmt > 0) totals.push(['Diskon', `-${fmt(data.discountAmt, data.currency)}`])
  ...
  for (const [label, value] of totals) {
    ...
    text(label + ' '.repeat(spaces) + value); nl()
  }
  ...
  dblHeight(true)
  align('right')
  text(`TOTAL: ${fmt(data.total, data.currency)}`); nl()
  dblHeight(false)
  bold(false)
  align('left')
  ...
  if (data.paymentMethod) { text(`Metode: ${data.paymentMethod}`); nl() }
  if (data.points) { text(`Poin: +${data.points}`); nl() }

  text('-'.repeat(32)); nl()

  // Footer
  align('center')
  if (data.receiptNote) { text(data.receiptNote.substring(0, 48)); nl() }
...
export async function printReceiptSerial(data: ReceiptData): Promise<{ ok: boolean; error?: string }> {
...
export function buildReceiptLines(data: ReceiptData): ReceiptLine[] {
  ...
  if (data.storeAddress) lines.push({ type: 'text', text: data.storeAddress, align: 'center', size: 'small' })
  if (data.storePhone) lines.push({ type: 'text', text: data.storePhone, align: 'center', size: 'small' })
  if (data.taxId) lines.push({ type: 'text', text: `Tax ID: ${data.taxId}`, align: 'center', size: 'small' })
  ...
  if (data.customerName) lines.push({ type: 'text', text: `Customer: ${data.customerName}`, size: 'small' })
  lines.push({ type: 'divider' })

  for (const item of data.items) {
    lines.push({ type: 'item', left: item.name, right: fmt(item.subtotal, data.currency) })
    lines.push({ type: 'text', text: `  ${item.qty} × ${fmt(item.price, data.currency)}`, size: 'small' })
  }

  lines.push({ type: 'divider' })
  lines.push({ type: 'item', left: 'Subtotal', right: fmt(data.subtotal, data.currency) })
  if (data.discountAmt && data.discountAmt > 0)
    lines.push({ type: 'item', left: 'Discount', right: `-${fmt(data.discountAmt, data.currency)}` })
  ...
  if (data.paymentMethod) lines.push({ type: 'text', text: `Payment: ${data.paymentMethod}`, size: 'small' })
  if (data.points) lines.push({ type: 'text', text: `Points earned: +${data.points}`, size: 'small' })

  lines.push({ type: 'divider' })
  if (data.receiptNote) lines.push({ type: 'text', text: data.receiptNote, align: 'center', size: 'small' })
