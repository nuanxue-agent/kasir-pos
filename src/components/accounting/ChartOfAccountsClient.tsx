    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/30 backdrop-blur-sm">
      <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-3xl shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          <h2 className="text-base font-bold text-stone-800">Tambah Akun</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-stone-100" aria-label="Tutup">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-stone-500 mb-1.5 block">Kode Akun * <span className="font-normal text-stone-400">(4 digit)</span></label>
            <input
              value={form.code}
          ...
          <div>
            <label className="text-xs font-semibold text-stone-500 mb-1.5 block">Saldo Awal</label>
            <input
        ...
        <div className="p-4 flex gap-3 border-t border-stone-100">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-stone-100 text-stone-600 text-sm font-semibold hover:bg-stone-200">
            Batal
          </button>
          <button
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl p-5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-stone-800">Hapus Akun?</p>
            <p className="text-xs text-stone-400">{account.code} – {account.name}</p>
          </div>
        </div>
        {hasBalance && (
...
function AccountRow({
      ...
      <>
        <div className="flex items-center px-4 py-2 gap-2 bg-amber-50/50">
          <input
            ...
            className="font-mono text-xs font-bold text-stone-700 w-16 bg-white border border-stone-200 rounded-lg px-2 py-1 focus:outline-none focus:border-amber-400"
            maxLength={4}
            inputMode="numeric"
          ...
          <input
            ...
            className="flex-1 text-sm text-stone-800 bg-white border border-stone-200 rounded-lg px-2 py-1 focus:outline-none focus:border-amber-400"
          ...
          <button onClick={cancelEdit} className="p-1.5 rounded-lg hover:bg-stone-100" aria-label="Batal">
  ...
  return (
    <>
      <div className="flex items-center px-4 py-3 hover:bg-stone-50 transition-colors group">
        <span className="font-mono text-xs font-bold text-stone-500 w-14 shrink-0">{account.code}</span>
        <span className="text-sm font-medium text-stone-700 flex-1">{account.name}</span>
        <span className={cn('text-xs font-semibold px-2 py-0.5 rounded-lg shrink-0 mr-3', cfg.bg, cfg.color)}>
          {cfg.label}
        </span>
        <span className="text-xs text-stone-400 shrink-0 w-24 text-right mr-3">
          {formatCurrency(account.balance, currency)}
        </span>
        {!account.isSystem && (
    ...
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-5 pb-24 lg:pb-6">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={cn('px-3 py-1.5 rounded-xl text-xs font-semibold transition-all',
            !typeFilter ? 'bg-amber-500 text-white' : 'bg-stone-50 text-stone-500 border border-stone-200 hover:bg-stone-100')}
        >
          Semua
        </button>
        {ACCOUNT_TYPES.map(t => (
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[...Array(8)].map((_, i) => <div key={i} className="h-12 bg-stone-50 animate-pulse rounded-2xl" />)}
        </div>
      ) : (
