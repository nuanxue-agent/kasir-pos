  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-1)]">Discounts</h1>
          <p className="text-[var(--text-2)] mt-1 text-sm">Manage coupon codes and promotions</p>
        </div>
        <button
        ...
        <div className="space-y-3">
          {discounts.map((d: Discount) => (
            <div key={d.id} className={cn(
              'bg-[var(--bg-muted)] rounded-xl p-4 flex items-center gap-4 border',
              d.active ? 'border-[var(--border)]' : 'border-[var(--border)] opacity-60'
            )}>
              {/* Badge */}
              <div className={cn(
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-medium text-[var(--text-1)]">{d.name}</p>
                  {!d.active && <span className="text-xs bg-stone-200 text-[var(--text-2)] px-2 py-0.5 rounded-full">Nonaktif</span>}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="p-2 text-[var(--text-2)] hover:text-[var(--text-1)] hover:bg-stone-200 rounded-lg transition-colors"
                >
                  <Pencil size={15} />
                </button>
                <button
                  onClick={() => deleteMutation.mutate(d.id)}
                  className="p-2 text-[var(--text-2)] hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors"
                >
                  <Trash2 size={15} />
                </button>
              </div>
...
function DiscountFormModal({ storeId, currency, discount, onClose, onSuccess }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--bg-card)] rounded-xl w-full max-w-md border border-[var(--border)] p-6 space-y-4">
        <h2 className="text-lg font-semibold text-[var(--text-1)]">{discount ? 'Edit Discount' : 'New Discount'}</h2>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="flex-1 py-2 rounded-xl border border-[var(--border)] text-[var(--text-2)] hover:text-[var(--text-1)] text-sm transition-colors">Cancel</button>
...
const ic = 'w-full bg-[var(--bg-subtle)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--text-1)] text-sm focus:outline-none focus:ring-2 focus:ring-amber-400'
