...
export function BottomNav({ modules }: BottomNavProps) {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-stone-100">
      <div className="flex items-stretch h-16">
        {navItems.map(({ label, href, icon: Icon, module: mod }) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                ...
                isPOS ? 'mt-1' : '',
                active ? (isPOS ? 'text-amber-600' : 'text-amber-600') : 'text-stone-400'
              )}>
                {label}
              </span>
              {active && !isPOS && (
