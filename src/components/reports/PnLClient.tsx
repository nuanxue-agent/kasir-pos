...
function PctBadge({ value }: { value: number | null }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 text-xs font-semibold ${pos ? 'text-emerald-600' : 'text-red-500'}`}
    >
...
function LineRow({
  ...
  return (
    <div
      className={`flex items-center justify-between px-5 py-2.5 ${bold ? 'bg-[var(--bg-subtle)]/80' : 'hover:bg-[var(--bg-subtle)]/40'} transition-colors`}
    >
      <span
        className={`text-sm ${indent ? 'pl-4' : ''} ${bold ? 'font-bold text-[var(--text-1)]' : 'text-[var(--text-2)]'}`}
      >
        {label}
      </span>
      <div className="flex items-center gap-4">
        ...
        {prevAmount !== undefined && (
          <span className="hidden w-28 text-right font-mono text-xs text-stone-300 sm:block">
            {formatCurrency(prevAmount, currency)}
          </span>
        )}
        <span
          className={`w-32 text-right font-mono text-sm ${bold ? 'font-bold' : ''} ${valueClass}`}
        >
...
export function PnLClient({ storeId, currency }: PnLClientProps) {
  ...
  const skeleton = () => (
    <div className="animate-pulse space-y-2">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="h-10 rounded-xl bg-[var(--bg-subtle)]" />
      ))}
    </div>
  )
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 p-4 pb-24 sm:p-6 lg:pb-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-[var(--text-1)] sm:text-2xl">Laba Rugi</h1>
        <p className="mt-0.5 text-sm text-[var(--text-3)]">
          Profit &amp; Loss — performa keuangan dalam periode tertentu
        </p>
      </div>
      ...
      <div className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-sm">
        ...
        <div className="flex gap-2">
          ...
          <ExportButton
            ...
            currency={currency}
          />
          <PrintButton title={`Laba Rugi ${from} s/d ${to}`} />
        </div>
      </div>
      ...
      {!isLoading && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-sm">
            <p className="mb-1 text-xs font-semibold text-[var(--text-3)]">Pendapatan</p>
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-sm">
            <p className="mb-1 text-xs font-semibold text-[var(--text-3)]">Laba Kotor</p>
            <p className="text-lg font-bold text-[var(--text-1)]">
              {formatCurrency(t.grossProfit, currency)}
            </p>
            <span className="text-xs text-[var(--text-3)]">Margin {grossMargin.toFixed(1)}%</span>
          </div>
          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-sm">
            <p className="mb-1 text-xs font-semibold text-[var(--text-3)]">Biaya Operasional</p>
            <p className="text-lg font-bold text-red-500">
              -{formatCurrency(t.operatingExpenses, currency)}
            </p>
            <PctBadge value={pctChange(t.operatingExpenses, pt.operatingExpenses)} />
          </div>
          <div
            className={`rounded-xl border p-4 shadow-sm ${t.netProfit >= 0 ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50'}`}
          >
            <p className="mb-1 text-xs font-semibold text-[var(--text-3)]">Laba Bersih</p>
            <p
              className={`text-lg font-bold ${t.netProfit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
            >
      ...
      {isLoading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-sm">
          ...
          <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg-subtle)] px-5 py-3">
            <span className="text-xs font-bold tracking-wide text-[var(--text-2)] uppercase">
              Akun
            </span>
            <div className="flex items-center gap-4">
              <span className="hidden w-28 text-right text-xs text-stone-300 sm:block">
                Periode Lalu
              </span>
              <span className="w-32 text-right text-xs font-bold text-[var(--text-2)]">
                Periode Ini
              </span>
            </div>
          </div>
          ...
          <div className="border-b border-[var(--border)]">
            <div className="bg-amber-50/60 px-5 py-2">
              <span className="text-xs font-bold tracking-wide text-amber-600 uppercase">
                Pendapatan
              </span>
            </div>
          ...
          {/* COGS */}
          <div className="border-b border-[var(--border)]">
            <div className="bg-[var(--bg-subtle)]/60 px-5 py-2">
              <span className="text-xs font-bold tracking-wide text-[var(--text-2)] uppercase">
          ...
          <div className="border-b border-[var(--border)]">
            ...
            <LineRow
              ...
              amount={t.operatingExpenses}
              prevAmount={pt.operatingExpenses}
              currency={currency}
              bold
            />
          </div>
